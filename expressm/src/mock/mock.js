...
//入职日期接口
Mock.mock('http://localhost:8080/in','get',()=>{       //不传参最适合用get
    return {
        code:200,
        success:true,
        message:'请求成功',
...
Mock.mock('http://localhost:8080/orderList', 'post', (req) => {
    ...
    return {
        ...
        message: "成功",
        data: Mock.mock({
            [`list|${pageSize}`]: [{  //这是mock的语法：list|100随机生成100项
                'id|+1': 10000,//订单号  mock语法：自增一
                'status|1': [1,2,3,4],//订单状态 1待审核 2已审核 3审核通过 4审核拒绝  mock语法 随机取一项
                'date': Mock.Random.date(),//下单时间    mock语法：随机生成时间
                'name|1': ["诺来科技有限公司","辉华股份有限公司","川聚物流有限公司","成越材料有限公司","聚博纺织有限公司"],//客户名称
                'start': Mock.Random.city(true),//起始城市       mock语法：随机生成城市
                'end': Mock.Random.city(true),//目的城市,           mock语法：随机生成城市
                'cargo|1': ["日用品","纺织品","生鲜","建材","电器"],//货物名称   mock语法：随机生成物品
                'count': Mock.Random.integer(10, 200),//件数      mock语法：随机生成件数
                'unit|1': ["方","吨"],//单位
...
//删除订单
Mock.mock('http://localhost:8080/delete','post',(req)=>{
        ...
        data: Mock.mock({
            [`list|${pageSize}`]: [{
                ...
                'plateNumber|1': ["京A12345","苏C66666","鲁B45678"],//车牌号
                 "driver":Mock.Random.cname(), //mock语法：随机生成一个人名
                 "tel|1":[18888888888,15577896554,15789654785,13698745269],
                 "percent|1":[37,22,89,65,80,74,56]//进度
    ...
    return {
        ...
        data: Mock.mock({
            [`list|${pageSize}`]: [{  //这是mock的语法：list|100随机生成100项
                ...
                "needRecive|1":[1,2],//需要接货 1需要 2不需要
                "price":Mock.Random.integer(5000,50000),//运费
                "plateNumber|1":['京A88888','沪A999999','粤A777777'],
                "tel|1":[18888888888,15577896554,15789654785,13698745269],
                 "percent|1":[37,22,89,65,80,74,56]//进度 
            }],
            "total": 47  //总数据一共多少条
...
//根据token获取权限名字
// http://localhost:8080/waybillList
...
//承运商管理
Mock.mock('http://localhost:8080/carriersData', 'post', (req) => {
    const { page, pageSize } = JSON.parse(req.body);
    console.log("接口接收到参数:",page,pageSize)
    return {
        code: 200,
